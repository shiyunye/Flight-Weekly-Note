...
def format_percentage_values(
    ...
    # pre-scan header to tag columns
    ...
    for c in header_cells:
        ...
        col_is_conversion.append("conversion" in htxt)
            ...
            for paragraph in getattr(cell, "paragraphs", []):
                for run in getattr(paragraph, "runs", []):
                    ...
                    if not value.endswith("%"):
                        continue  # only handle percentage-looking values
                    ...
                    # format with/without leading '+' :
                    if num_value > 0:
                        if is_conversion_col:
                            # exception: for Conversion columns, no leading '+'
                    ...
                    # color rule:
                    #   if header contains any keyword -> BLACK
...
def create_summary_table(word_document,df_summary):
    ...
    index_logo_path = '../Screenshots/weeklynote/priceline.jpg'
    ...
    icons_and_labels = [
        ('../Screenshots/weeklynote/Tickets.jpg', 'Tickets', 2, 3),
        ('../Screenshots/weeklynote/Revenue.jpg', 'Revenue', 4, 5),
        ('../Screenshots/weeklynote/Normalized Tickets.jpg', 'Normalized Tickets', 6, 7)
    ]
    ...
    paragraph = cell.paragraphs[0]
    paragraph.add_run().add_picture('../Screenshots/weeklynote/dau.jpg', width=Inches(0.4), height=Inches(0.4))
    paragraph.add_run("\n Daily Active Users")
    set_font(paragraph, font_name="Montserrat", font_size=Pt(7), bold=False)
    center_text_in_cell(cell)
    ...
    paragraph = cell.paragraphs[0]
    paragraph.add_run().add_picture('../Screenshots/weeklynote/tsa.jpg', width=Inches(1), height=Inches(0.33))
    paragraph.add_run("\n Market Share")
    set_font(paragraph, font_name="Montserrat", font_size=Pt(7), bold=False)
    center_text_in_cell(cell)

    # Add "vs. Last Year" and "vs. Plan" labels in the header row
    last_year_logo_path = '../Screenshots/weeklynote/lastyear.jpg'
    plan_logo_path = '../Screenshots/weeklynote/plan.jpg'
...
def create_others_table(word_document,df_others):
    ...
    rows, cols = (df_others.shape[0]+2, df_others.shape[1]+3)
    table = word_document.add_table(rows=rows, cols=cols, style='Table Grid')
    set_table_style(table, border_color="000000", border_width=8, fill_color="FFFFFF")

        
    # Add header logos
    header_logo_path_1 = '../Screenshots/weeklynote/priceline.jpg'
    header_logo_path_2 = '../Screenshots/weeklynote/expedia.jpg'
    ...
    icons_and_labels_col1 = [
        ('../Screenshots/weeklynote/Market Share.jpg', 'Market Share', 2, 7),
    ...
    # Priceline icon
    priceline_cell = table.cell(2, 2)
    priceline_paragraph = priceline_cell.paragraphs[0]
    priceline_paragraph.add_run().add_picture(
        '../Screenshots/weeklynote/priceline.jpg',
        width=Inches(0.48),
        height=Inches(0.21)
    )
    center_text_in_cell(priceline_cell)

    # Agoda icon
    agoda_cell = table.cell(3, 2)
    agoda_paragraph = agoda_cell.paragraphs[0]
    agoda_paragraph.add_run().add_picture(
        '../Screenshots/weeklynote/agoda.jpg',
        width=Inches(0.5),
        height=Inches(0.26)
    )
    center_text_in_cell(agoda_cell)

    # B.com icon
    bcom_cell = table.cell(4, 2)
    bcom_paragraph = bcom_cell.paragraphs[0]
    ...
    # BHI icon
    bhi_cell = table.cell(5, 2)
    bhi_paragraph = bhi_cell.paragraphs[0]
    bhi_paragraph.add_run().add_picture(
        '../Screenshots/weeklynote/BHI.jpg',
        width=Inches(0.5),
        height=Inches(0.12)
    )
    set_font(bhi_paragraph, font_name="Montserrat", font_size=Pt(10), bold=False)
...
def create_roi_table(word_document, df):
    ...
    logos = [
        {'start_col': 2, 'end_col': 4, 'path': '../Screenshots/weeklynote/dau.jpg',        'title': 'DAU'},
        {'start_col': 5, 'end_col': 7, 'path': '../Screenshots/weeklynote/conversion.jpg', 'title': 'Conversion'},
        {'start_col': 8, 'end_col': 10, 'path': '../Screenshots/weeklynote/roi.jpg',        'title': 'ROI'},
    ]

    # Build table (11x11 as in your original)
    table = word_document.add_table(rows=12, cols=11, style='Table Grid')
    ...
    icons_and_labels_col1 = [
        ('../Screenshots/weeklynote/Web Marketing.jpg', 'Web Marketing', 3, 4),
        ('../Screenshots/weeklynote/Shop PPC.jpg',       'Shop PPC',      5, 9),
    ]
    for icon_path, label, start_row, end_row in icons_and_labels_col1:
    ...
    ('../Screenshots/weeklynote/skyscanner.jpg', '', 8, 8, 1.0, 0.18), 
    ('../Screenshots/weeklynote/other.jpg', 'Other', 9, 9, 0.2, 0.2), 
    ('../Screenshots/weeklynote/Affiliate.jpg', 'Affiliate', 10, 10, 0.4, 0.4)] 
