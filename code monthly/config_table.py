...
def format_color_values(
    ...
    # --- tag keyword columns from header row ---
    ...
    # --- walk all cells except header row ---
    for r_idx, row in enumerate(table.rows):
        ...
        for c_idx, cell in enumerate(row.cells):
            ...
            # keyword columns: do nothing at all
            if is_key_col:
                continue
            ...
            for paragraph in getattr(cell, "paragraphs", []):
                for run in getattr(paragraph, "runs", []):
                    ...
                    # --- format text ---
                    if num_value >= 0:
                        run.text = f"+{num_value:.1f}%"
                        run.font.color.rgb = RGBColor(106, 168, 79)  # green
                    else: 
                        run.text = f"{num_value:.1f}%"
                        run.font.color.rgb = RGBColor(255, 0, 0)    # red

                    # optional: only non-keyword cols bold
                    run.font.bold = True
...
def create_others_table(word_document,df_others):
    ...
    columns = ['Actual', 'YoY', 'YoY PW', 'Actual', 'YoY', 'YoY PW']
    for j, name in enumerate(columns):
