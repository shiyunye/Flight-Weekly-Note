...
def calculate_metrics(data,end_date, pp_end_date, group_col, filters=None, suffix=""):
    """
    Generalized function to calculate Net Contr, YoY, and YoY PW metrics.

    Parameters:
    - data: DataFrame to process.
    - end_date: Current week ending date.
    - pp_end_date: Previous week ending date.
    - group_col: Column to group by (e.g., 'company', 'carrier', 'source_group', etc.).
    - filters: Dictionary of column-value pairs to filter the data (optional).
    - suffix: Suffix to append to column names.
    ...
    df = pd.DataFrame()
    df['Net Contr'] = grouped_current['net_contribution_cy'].sum().astype(int)
    df['Net Contr_cwly'] = grouped_current['net_contribution_ly'].sum().astype(int)
    df['Net Contr_PW'] = grouped_previous['net_tkts_cy'].sum().astype(int)
    df['Net Contr_PWly'] = grouped_previous['net_contribution_ly'].sum().astype(int)

    # Add totals
    df.loc['Total', 'Net Contr':'Net Contr_PWly'] = [
        df['Net Contr'].sum(), df['Net Contr_cwly'].sum(),
        df['Net Contr_PW'].sum(), df['Net Contr_PWly'].sum()
    ]

    # Calculate YoY and YoY PW
    df['YoY'] = ((df['Net Contr'] / df['Net Contr_cwly'] - 1) * 100).round(1).astype(str) + '%'
    df['YoY PW'] = ((df['Net Contr_PW'] / df['Net Contr_PWly'] - 1) * 100).round(1).astype(str) + '%'

    # Select and format columns
    df = df[['Net Contr', 'YoY', 'YoY PW']]
    df['Net Contr'] = df['Net Contr'].apply(format_number)
...
def calculate_channel_metrics(df_pricelince_b2c_standalone,end_date, pp_end_date):
    df_app = calculate_metrics(
        df_pricelince_b2c_standalone,end_date, pp_end_date, 'source_group', filters={'application': 'App'}, suffix='_App'
    )
    df_desk_mweb = calculate_metrics(
        df_pricelince_b2c_standalone,end_date, pp_end_date, 'source_group', filters={'application': 'Desk/MWEB'}, suffix='_Desk/MWEB'
    )
    df_total = calculate_metrics(
        df_pricelince_b2c_standalone,end_date, pp_end_date, 'source_group', suffix='_Total'
    )

    df_channel = pd.concat([df_app, df_desk_mweb, df_total], axis=1)
    df_channel = df_channel.reindex([
        'Direct', 'SEM Core', 'SEM Brand', 'Shop PPC', 'Affiliate', 'Email', 'Meta', 'Total'
    ]).fillna('')
    return df_channel

def calculate_source_metrics(df_pricelince_b2c_standalone,end_date, pp_end_date):
    df_published = calculate_metrics(
        df_pricelince_b2c_standalone,end_date, pp_end_date, 'gds_bkng_name_grp', filters={'fare_type_group': 'Published'}, suffix='_Published'
    )
    df_private = calculate_metrics(
        df_pricelince_b2c_standalone,end_date, pp_end_date, 'gds_bkng_name_grp', filters={'fare_type_group': 'Private'}, suffix='_Private'
    )
    df_total = calculate_metrics(
        df_pricelince_b2c_standalone,end_date, pp_end_date, 'gds_bkng_name_grp', suffix='_Total'
    )
